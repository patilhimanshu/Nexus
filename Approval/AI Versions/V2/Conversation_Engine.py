...
def handle_message(user_message):
    ...
    reply, and handles all the memory/trait/reasoning side effects.
    This is what Charlie's main loop calls instead of the old
    detect_intent().
    ...
    # If the reasoning layer flagged this as a task, log it loudly —
    # this is the seam where 0.06 Planner will eventually hook in to
    # actually call db.create_task() instead of just printing.
    if reasoning["intent"] == "task" and reasoning["priority"] == "high":
        print(f"[REASONING] High-priority task detected: {reasoning['action']}")
